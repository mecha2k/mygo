...
def do_self_play(board_size, agent_filename, num_games, temperature, experience_filename, gpu_frac):
...
def main():
    ...
    combined_buffer = reinforce.load_experience(h5py.File(filename))
    for fname in other_filenames:
        next_buffer = reinforce.load_experience(h5py.File(fname))
