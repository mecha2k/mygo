...
class State(object):
    ...
    def winner(self):
        # 3 in a row
        for row in (0, 1, 2):
            if self.cell(row, 0) != Cell.EMPTY and self.cell(row, 0) == self.cell(
                row, 1
            ) == self.cell(row, 2):
                return self.cell(row, 0)
        # 3 in a col
        for col in (0, 1, 2):
            if self.cell(0, col) != Cell.EMPTY and self.cell(0, col) == self.cell(
                1, col
            ) == self.cell(2, col):
                return self.cell(0, col)
        # TL to BR diagonal
        if self.cell(0, 0) != Cell.EMPTY and self.cell(0, 0) == self.cell(1, 1) == self.cell(2, 2):
            return self.cell(0, 0)
        # TR to BL diagonal
        if self.cell(0, 2) != Cell.EMPTY and self.cell(0, 2) == self.cell(1, 1) == self.cell(2, 0):
    ...
    def is_over(self):
        if self.winner() != Cell.EMPTY:
            return True
        return all(self.cell(row, col) != Cell.EMPTY for row in (0, 1, 2) for col in (0, 1, 2))
    ...
    def apply_move(self, row, col):
        ...
        return State(next_cells, Cell.O if self.next_to_play == Cell.X else Cell.X)
...
class Q(object):
    def __init__(self):
        ...
        self._table = 0.5 + 0.1 * np.random.randn(self._max_state_index, self._max_action_index)
...
def print_game(state):
    rows = "ABC"
    for row in (0, 1, 2):
        print(rows[row] + "  " + " | ".join(state.cell(row, col).value for col in (0, 1, 2)))
    print("   1   2   3")
    print("\n")
...
class Agent(object):
    ...
    def _learn(self, state):
        ...
        reward = x_reward if self._prev_state.next_to_play == Cell.X else -1 * x_reward
        next_reward = max(self._q(state, next_move) for next_move in state.get_actions())
        delta = reward + self._discount * next_reward - self._q(self._prev_state, self._prev_action)
        self._q.increment(self._prev_state, self._prev_action, self._learning_rate * delta)
...
def self_play(q, learning_rate, temperature):
    player_x = Agent(q, learning_rate=learning_rate, discount=1.0, temperature=temperature)
    player_o = Agent(q, learning_rate=learning_rate, discount=1.0, temperature=temperature)
    game_state = new_game()
...
def main():
    ...
    parser.add_argument("--train", "-t", type=int, help="Number rounds of self-play.")
    parser.add_argument(
        "--input_file",
        "-i",
        help="File containing initial Q matrix. If unset, " "will initialize with random weights.",
    )
    parser.add_argument("--output_file", "-o", help="File to save updated Q matrix.")
