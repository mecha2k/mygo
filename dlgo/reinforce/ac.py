...
class ACAgent(Agent):
    ...
    def select_move(self, game_state):
        ...
        for point_idx in ranked_moves:
            ...
            if game_state.is_valid_move(goboard.Move.play(point)) and not is_point_an_eye(
                game_state.board, point, game_state.next_player
            ):
                if self.collector is not None:
                    self.collector.record_decision(
                        state=board_tensor, action=point_idx, estimated_value=estimated_value
                    )
    ...
    def train(self, experience, lr=0.1, batch_size=128):
        ...
        self.model.fit(
            experience.states, [policy_target, value_target], batch_size=batch_size, epochs=1
        )
