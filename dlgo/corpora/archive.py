...
__all__ = ["SGF", "SGFLocator", "find_sgfs"]
...
class TarballSGFLocator(SGFLocator):
    def __init__(self, tarball_path, archive_filename, archive_path):
        super().__init__(archive_path, archive_filename)
...
def tarball_iterator(tarball_path):
    ...
    sgf_names = [
        tf_entry.name for tf_entry in tf if tf_entry.isfile and tf_entry.name.endswith(".sgf")
    ]
    sgf_names.sort()
    tf.extractall(tempdir)
    try:
        yield [
            SGF(SGFLocator(tarball_path, sgf_name), open(os.path.join(tempdir, sgf_name)).read())
