...
def interpret_point_list(values, context):
    ...
    Raises ValueError if the dataprocess is otherwise malformed.
...
def serialise_AP(value, context):
    ...
    return sgf_grammar.compose(
        serialise_simpletext(application, context), serialise_simpletext(version, context)
    )
...
def serialise_ARLN_list(values, context):
    ...
    """
    return [
        b":".join((serialise_point(p1, context), serialise_point(p2, context))) for p1, p2 in values
    ]
...
def serialise_LB_list(values, context):
    ...
    """
    return [
        b":".join((serialise_point(point, context), serialise_simpletext(text, context)))
