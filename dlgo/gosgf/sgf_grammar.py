"""Parse and serialise SGF dataprocess.
...
def tokenise(s, start_position=0):
    """Tokenise a string containing SGF dataprocess.
...
def _parse_sgf_game(s, start_position):
    ...
    try:
        ...
        raise ValueError("unexpected end of SGF dataprocess")
...
def parse_sgf_game(s):
    ...
    if game_tree is None:
        raise ValueError("no SGF dataprocess found")
    return game_tree
...
def parse_sgf_collection(s):
    ...
    Ignores non-SGF dataprocess before the first game, between games, and after the
